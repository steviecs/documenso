...
export const run = async ({
  ...
  io.logger.info('[send-reminder-email] render complete', {
  ...
  if (!html || html.length < 100) {
    throw new Error(
      `[send-reminder-email] rendered HTML is suspiciously short (${html?.length ?? 0} chars) — refusing to send blank email`,
    );
  }
